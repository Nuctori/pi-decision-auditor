...
- **只审真实的**。纯咨询轮（无代码、无决策）审计者快速退出、零注入。审空轮次是走形式，走形式会教会审计者盖章放行。
- **交付把关，而非每轮把关**。常规轮异步——审计者在后台工作，你永不被阻塞。门禁只在关键时刻收紧：提交 / 发布 / merge / 部署。
...
```text
每次 agent_end（有真实产物时）
  └─ 真实产物？（git 改动，排除 .pi 状态；或对话增量）→ spawn 审计者
     · 审计者 AI 判定（第零步）：纯咨询 / 无决策 / 无产物 → 快速退出、零注入
...
| **fresh spawn 生命周期** | 审计结束 run 即结束——无常驻进程、无残留；纯咨询轮 spawn 后快速退出（零注入） |
