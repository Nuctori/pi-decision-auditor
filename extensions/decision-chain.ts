...
function buildIncrementalAuditTask(cwd: string, runId: string): string {
	...
	lines.push(
		"1. **审计对象 = 上次审计后的全部产物**：① 自**上次真实审计完成时刻**之后**已提交**的改动——窗口起点 = state.json 的 signature.at（上次审计者签名的 epoch ms，正常收尾时与 lastAuditAt 同步）；**但 signature.status 为 'passed-with-warning'（交付轮超时降级/连续 blocked 放行）或 'failed'（spawn 失败，v1.0.33 补：失败写入同样在 agent_end、晚于提交）时，signature.at 是扩展写入的降级/失败时间而非真实审计完成时间——晚于产物提交 → `git log --since=signature.at` 空窗口、未审产物被跳过（v1.0.31 实证：审计者死于流错误降级，4c3d997 靠 git show HEAD 兜底才过审），此时窗口起点回退 lastAuditAt÷1000（上次真实审计边界）**；无 signature 或回退值 ≤0（从未真实审计过）→ 首次审计语义：用最近 20 个提交兜底；用 `git log --since=<窗口起点÷1000 epoch 秒>`（**13 位毫秒必须 ÷1000 转秒**；或用 `git log --since='YYYY-MM-DD HH:MM:SS'` ISO 时间，或 `git log --oneline -20`）定位窗口内提交，`git show <commit>` 逐个审（**勿用 `--since=0`——git approxidate 怪癖会得空窗口**）。**窗口兜底自查（防异步审计空洞，v1.0.34）**：`git log --since=<窗口起点>` 为空且存在 signature（非首次审计）时，对照 signature.head（上次审计的产物基线）——**HEAD ≠ signature.head = 存在未审提交**（提交落库于上轮异步审计运行期间 → commit date 早于上轮签名时间 → 被两轮窗口同时排除），用 `git show HEAD`（结合 `git log --oneline -5`）逐个核对未审提交后再判定。② 当前未提交 diff（`git diff`）。按五维度逐项进攻①原子性 ②正确性 ③一致性 ④内聚 ⑤完备（详见维度定义）。**窗口内无任何产物（无新提交且无未提交 diff；HEAD == signature.head 或首次审计）**：审决策质量——本轮提取的决策条目是否自足（Context 可验证/Decision 明确/Rationale 由 Context 推出/Alternatives 认真考虑）？决策是否服务于推导出的目标（有无漂移）？",
	);
...
const DELIVERY_ANGLES: Array<{
...
}> = [
	{
		...
		prompt: (cwd, runId) =>
			...
			`审自上次真实审计（.pi/decision-auditor/state.json 的 signature.at——上次审计完成时间；**signature.status 为 passed-with-warning（超时降级/连续 blocked 放行）或 failed（spawn 失败）时 signature.at 是降级/失败写入时间 → --since 空窗口、未审产物被跳过，窗口起点回退 lastAuditAt÷1000；无 signature 或回退值 ≤0（从未真实审计过）→ 首次审计语义：最近 20 个提交**，勿用 --since=0——git approxidate 怪癖会得空窗口）之后**已提交**的改动（git log --since + git show 逐个提交，13 位毫秒必须 ÷1000 转秒）与当前未提交 diff（git diff）；**窗口兜底：git log --since 为空且 HEAD ≠ signature.head（上次审计基线）→ 用 git show HEAD 逐个核对**（提交落库于上轮异步审计运行期间会被两轮窗口同时排除），以及 ${chainPath(cwd)}（默认 .pi/decision-auditor/chain.md，PI_PAIR_CHAIN_PUBLIC=1 时在 docs/decisions/chain.md）：\n` +
