...
const DELIVERY_ANGLES: Array<{
...
}> = [
	{
		...
		prompt: (cwd, runId) =>
			...
			`【低价值窗口轻量退出（v1.0.45 收敛纪律，与 L1 同构）】窗口内提交/未提交改动仅文档/格式（.md/CHANGELOG/README/package.json 版本号，无代码逻辑改动）且无新决策 → 输出简短确认（1-2 行概述文档一致性核对结果）即可，不做全维度深度审查。**例外**：文档改动隐含行为/设计决策（CHANGELOG 描述功能、README 改架构说明）→ 照常深度审查。`,
	},
];
